...
def _check_allowed_routes_caller_permission(
    ...
    *,
    ...
    Raw-body call sites pass `is_explicit="allowed_routes" in data.model_fields_set`
    so a caller that omits the field (model default flows through) is distinct
    from one that sends any explicit value.
    ...
    body, so `is_explicit` stays False and the safe-preset carve-out below
    accepts any list of tokens in `_NON_ADMIN_SAFE_ALLOWED_ROUTES_PRESETS`.
    """
    if not is_explicit and not allowed_routes:
        return
    if user_api_key_dict.user_role == LitellmUserRoles.PROXY_ADMIN.value:
        return
    if (
...
async def generate_key_fn(
    ...
    try:
        ...
        _check_allowed_routes_caller_permission(
            ...
            user_api_key_dict=user_api_key_dict,
            is_explicit="allowed_routes" in data.model_fields_set,
        )
        _check_passthrough_routes_caller_permission(
            data=data,
            user_api_key_dict=user_api_key_dict,
